...
use rand::random;
use sensor_fuse::{
    sensor_core::alloc::{AsyncCore, MySensorCore, MyWriter},
    SensorObserveAsync, SensorWriteAsync, SensorWriter,
};
use std::{
    hint::black_box,
    sync::{
...
struct MyContentionEnvironment {
    reader_handles: Vec<tokio::task::JoinHandle<()>>,
    writer_handles: Vec<tokio::task::JoinHandle<()>>,
    writer: MyWriter<ContentionData, Arc<MySensorCore<ContentionData>>>,
    _runtime: Runtime,
}

impl MyContentionEnvironment {
    fn new(reader_count: usize, writer_count: usize) -> Self {
        let writer = MyWriter::new(ContentionData::default());
        let runtime = runtime::Builder::new_multi_thread()
            .worker_threads(4)
            .build()
            .unwrap();
    ...
    fn bench_individual_writes_req(&mut self, required_writes: usize, observer_spin_loops: usize) {
        let mut observer = self.writer.subscribe();
        block_on(self.writer.modify(|state| {
            state.test_version += 1;
            state.observers_completed.fetch_and(0, Ordering::Relaxed);
            state.writers_completed = 0;
            state.individual_writes_required = required_writes;
...
impl Drop for MyContentionEnvironment {
    fn drop(&mut self) {
        block_on(self.writer.modify(|state| {
            state.test_version = usize::MAX;
            true
...
fn my_r5_w5_o50_s50_observation(c: &mut BenchmarkGroup<WallTime>) {
    let mut env = MyContentionEnvironment::new(5, 5);

    c.bench_function("my_r5_w5_o50_s50_observation", |b| {
        b.iter(|| {
            env.bench_individual_observations_req(50, 50);
        });
    });
...
fn my_r5_w5_o50_s50_writes(c: &mut BenchmarkGroup<WallTime>) {
    let mut env = MyContentionEnvironment::new(5, 5);

    c.bench_function("my_r5_w5_o50_s50_writes", |b| {
        b.iter(|| {
            env.bench_individual_writes_req(50, 50);
        });
    });
...
fn my_r10_w1_o50_s50_writes(c: &mut BenchmarkGroup<WallTime>) {
    let mut env = MyContentionEnvironment::new(10, 1);

    c.bench_function("my_r10_w1_o50_s50_writes", |b| {
        b.iter(|| {
            env.bench_individual_writes_req(50, 50);
        });
    });
...
pub fn bench_reads() {
    ...
    my_r5_w5_o50_s50_observation(&mut group);
...
pub fn bench_writes() {
    ...
    my_r5_w5_o50_s50_writes(&mut group);
    ...
    my_r10_w1_o50_s50_writes(&mut group);
